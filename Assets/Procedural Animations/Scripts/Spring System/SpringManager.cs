...
public class SpringManager : MonoBehaviour
{
    ...
    private void Update()
    {
        bool isMoving = _inputManager.MoveInput.sqrMagnitude >= 0.001f;
    ...
    {
        // check if the recoil is completed before allowing another again, useful for non accumulative weapon
        if (IsFiring && SpringSystem.instance.GetPositionSpring("Recoiling").IsAtRest())
        {
    ...
    public void AddSway()
    {
        MotionOffset swayResult = GetSwayOffset(SwayMotionSettings, _inputManager.LookInput);
    ...
    private MotionOffset GetSwayOffset(SwaySettings settings, Vector2 lookInput)
    {
        // create a position based on current mouse position
        Vector3 swayPosition = new Vector3(
            -lookInput.x, -lookInput.y, 0f) * settings.amount;
